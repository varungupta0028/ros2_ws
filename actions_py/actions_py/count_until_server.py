...
class CountUntilServerNode(Node):
    ...
    def goal_callback(self, goal_request: CountUntil.Goal):
        ...
        #         self.get_logger().info("A goal is already active, refusing new goal!!! ")
        #         return GoalResponse.REJECT

        # Validate the goal request 
        if goal_request.target_number <= 0:
            self.get_logger().fatal("The goal is rejected ")
            return GoalResponse.REJECT
        
        
        # Policy: Preempt existing goal while receiving new goal 
        # with self.goal_lock_:
        #     if self.goal_handle_ is not None and self.goal_handle_.is_active:
    ...
    def execute_callback(self, goal_handle: ServerGoalHandle):
        ...
        # once done, set goal final state to abort or succeed
        goal_handle.succeed()
