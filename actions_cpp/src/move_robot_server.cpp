...
public:
    ...
    {   
        ...
        //it is necessary to write all three callback together to initialize the node
...
private:
    ...
    {   
        ...
        {
            RCLCPP_INFO(this->get_logger(), "Invalid position/Velocity, goal rejected");
            return rclcpp_action::GoalResponse::REJECT;
        }
        // Policy: To validate new goal and abort the existing goal and accept new one:
        {
        ...
        RCLCPP_INFO(this->get_logger(),"Accepting the new goal");
    ...
    void execute_goal(const std::shared_ptr<MoveRobotGoalHandle> goal_handle){
        ...
        while (rclcpp::ok())
        {   
            // To check if need to preempt the goal 
            {
                std::lock_guard<std::mutex> lock(mutex_);
